...
Next, we select the **Filters button** ![](images/filters.png){width="25" height="23"} from the Data tab. This opens the "Row FILTERS" view at the top of the Jamovi screen where we can add a filter called *Filter 1* (@fig-filter1_age). Let's say that we want to study only the adults from the participants.
...
Adding computed variables to a Jamovi spreadsheet is straightforward. Click on **Add** button available on the Data tab and then select **Append** from the **Computed Variable** list. (Alternatively, select the Compute button ![](images/compute_button.png){width="26" height="24"}). An empty column has been created at the end of our dataset (@fig-compute_append). The grey dot symbol in the right of the column header indicates that this is a computed variable.

```{r}
#| label: fig-compute_append
#| fig-align: center
#| out-width: "95%"
#| echo: false
#| fig-cap: Adding computed variables to a Jamovi spreadsheet.

knitr::include_graphics(here::here("images", "compute_append.png"))
```

To set up the computed variable, either double-click on the column header, or select Setup from the Data tab. This opens the "COMPUTED VARIABLE" view where we can set the name of the computed variable as BMI and specify the @eq-bmi in the formula box (note that we must divide the height by 100 and use the exponentiation operator \^ or \*\*) (@fig-compute_bmi):
...
#| out-width: "85%"
#| echo: false
#| fig-cap: Rounding up the computed variable.

knitr::include_graphics(here::here("images", "round.png"))
```
...
-   less than 18.5 `r fa("arrow-right", fill = "orange")` underweight

-   18.5 to \<25.0 `r fa("arrow-right", fill = "orange")` healthy weight

-   25.0 to \<30.0 `r fa("arrow-right", fill = "orange")` overweight

-   30.0 or higher `r fa("arrow-right", fill = "orange")` obesity

Click on **Add** button available on the Data tab and select **Append** from the **Transformed Variable** list. (Alternatively, select the Transform button ![](images/transform_button.png){width="26" height="24"}). An empty column has been created at the end of our dataset (@fig-transform_append). 

```{r}
#| label: fig-transform_append
#| fig-align: center
#| out-width: "95%"
#| echo: false
#| fig-cap: Adding transformed variables to a Jamovi spreadsheet.

knitr::include_graphics(here::here("images", "transform_append.png"))
```
...
```{r}
#| label: fig-create_new_transf
#| fig-align: center
#| out-width: "95%"
#| echo: false
#| fig-cap: Setting up the transformation of a variable.

knitr::include_graphics(here::here("images", "create_new_transf.png"))
```
...
```{r}
#| label: fig-transf_view
#| fig-align: center
#| out-width: "75%"
#| echo: false
#| fig-cap: Adding conditions.

knitr::include_graphics(here::here("images", "transf_view.png"))
```
...
First, select **`+ Add recode condition`**. Second, we need to populate the **boxes** with the information as follows (@fig-recode1): 
...
- In the next box, we will put our condition e.g. the BMI we want as the cut off point
- After the use, add your new label (here `'underweight'`). If we are using text we must enclose it in `'...'`
...
#| out-width: "75%"
#| echo: false
#| fig-cap: Adding the first condition.

knitr::include_graphics(here::here("images", "recode1.png"))
```
...
#| out-width: "75%"
#| echo: false
#| fig-cap: Adding more conditions.

knitr::include_graphics(here::here("images", "recode2.png"))
```
...
#| out-width: "75%"
#| echo: false
#| fig-cap: Adding the last label for the data that does not meet the above conditions.

knitr::include_graphics(here::here("images", "recode3.png"))
```
...
The red dot symbol in the right of the column header indicates that this is a transformed variable.
Additionally, we can edit existing transformations by selecting **Edit** (@fig-edit). 
...
#| out-width: "75%"
#| echo: false
#| fig-cap: The transformed variable is recognized by a red dot in the right of the column header

knitr::include_graphics(here::here("images", "edit.png"))
```
