...
We can add labels to numerically coded values of categorical variables. Double-click on the variable name `sex`. We will get an extra menu open at the top of the Jamovi screen @fig-labels:
...
Next, we select the Filters button ![](images/filters.png){width="25" height="23"} from the Data tab. This opens the **filter view** at the top of the Jamovi screen where we can add a filter called *Filter 1* (@fig-filter1). Let's say that we want to study only the adults from the participants.

![Adding a filter](images/filter1.png){#fig-filter1}


In order to access functions, press the ![](images/fx.png){width="23" height="21"} icon  in the filter settings and from `VARIABLES` double-click on `age`. Then we type the condition `age >= 18`.

Notice that a column named `Filter 1` has been added to the Spreadsheet view. Cells that meet the condition `age >= 18` are checked with a green tick ![](images/tick.png){width="24" height="21"}, while the rest have a red x symbol ![](images/x.png){width="24" height="21"}. Lines with an `X` are grayed out indicating that these observations are now outside of the current dataset. There is also a switch where we can activate ![](images/active.png){width="27" height="20"} or inactivate ![](images/inactive.png){width="27" height="20"} the filter. Finally, it is also possible to add new filters to create new subsets of data.
