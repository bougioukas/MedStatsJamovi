...
**a)** by using the `and` operator in 'Filter 1' which means that both conditions (adults and females) must be true at the same time. Therefore, we type the expression `age >= 18 and sex == 'female'` (@fig-filter1_age_and_sex):
...
**b)** by adding the second condition (i.e. `sex == "female"`) as another expression to Filter 1 (by clicking the small `+` beside the first ecxpression) (@fig-filter1_age_sex):
