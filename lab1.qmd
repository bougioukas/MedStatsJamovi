...
At this extra menu there is the **Levels** setup. We can set the labels that should appear for each level of that category.
Click on the number '0' in the Levels box to enter the label for this level. Change this from a ‘0’ to the word ‘male’. Click on the number '1' and change it to ‘female’.
...
In order to access functions, press the ![](images/fx.png){width="25" height="23"} icon in the filter settings and from `VARIABLES` double-click on `age` (or we just type in the variable name but if the variable has a space, we must use `'..'` around the variable name). Then type the condition `age >= 18` and press `ENTER` from the keyboard.
...
There is also a switch where we can activate ![](images/active.png){width="27" height="20"} or inactivate ![](images/inactive.png){width="27" height="20"} the filter. Finally, it is also possible to add new filters to create new subsets of data.
...
Note that in the expression we can also use double quotes (i.e. `age >= 18 and sex == "female"`) or even the coded value of the level (i.e. `age >= 18 and sex == 1`).
