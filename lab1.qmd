...
Adding computed variables to a Jamovi spreadsheet is straightforward. Click on **Add** button available on the Data tab (or Compute button ![](images/compute_button.png){width="26" height="24"}) and then select **Append** from the Computed Variable list . An empty column has been created at the end of our dataset (@fig-append). The grey dot symbol in the right of the column header indicates that this is a computed variable.
...
To set up the computed variable, either double-click on the column header, or select Setup from the data tab. This opens the "COMPUTED VARIABLE" view with the formula box where we can specify the formula @eq-bmi for the column that looks as follows (@fig-compute_bmi):
