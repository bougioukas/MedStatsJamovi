...
**- Step 1:** State the null hypothesis, $H_{0}$, and alternative hypothesis, $H_{1}$, based on the research question. We decide a non-directional $H_{1}$ (also known as two-sided hypothesis) whether we test for effects in both directions (most common), otherwise a directional (also known as one-sided) hypothesis.
...
We can calculate the p-value Using a statistical program such as Jamovi. In our example, the p-value is \<0.001. Since p-value \< α, $H_{0}$ is rejected. We can also estimate the 95%CI of mean as following:
