...
In this Lab, we will use the data from "LungCapacity" dataset that we have already seen in the previous chapter (@sec-lab9).
...
As before, there is a clear upward trend indicating that increase in `Age` intake tends to coincide with increase in `LungCap`. Moreover, the trend seems to be linear, so a straight line can capture the overall pattern.
...
![In the menu at the top, choose Analyze > Regression ![](images/reg_icon.png){width="35" height="33"} > Linear Regression.](images/linear2.png){#fig-linear2 width=85%}


The *Linear Regression dialogue box* opens (@fig-linear3). From the left-hand pane drag the variable `LunCap` into the *Dependent Variable* box and the variable `Age` into the *Covariates* box on the right-hand side, as shown below:

![The Linear Regression dialogue box options.](images/linear3.png){#fig-linear3 width=65%}


Additionally, from the *Model Coefficients* section tick the box "**Confidence interval**" in **Estimate** (@fig-linear4):

![The Model Coefficients dialogue box options.](images/linear4.png){#fig-linear4 width=75%}
...
Now, let's find the equation from the regression table in @fig-linear5. In the **Estimate** column are the intercept a=0.54 and the slope b=0.26 for age. Thus the equation of the regression line becomes:
...
Finally, the quality of our model is presented in @fig-linear6:

![The coefficient of determination. ](images/linear6.png){#fig-linear6 width=35%}
...
The regression coefficient of the `Age` (b=0.26) is significantly different from zero (p \< 0.001) and indicates that there's on average an increase of 0.26 liters ($95\%$CI: 0.24 to 0.27) in lung capacity for every 1 year increase in age. Note that the $95\%$CI does not include the hypothesized null value of zero for the slope.
:::
