...
library(readr)
data_birthweight <- read_csv(here("data", "data_birthweight.csv"))
...
(2) the **probability** corresponding to each possible value or value ranges (**probability distribution**).
:::
...
A normal distribution is the familiar "bell curve" and it's a way of formalizing a distribution where observations cluster around some central tendency. Observations farther from the central tendency occur less frequently (@fig-normal).
...
normal_dist("#522d80","deeppink") + 
  ...
  post_bg() +
    labs(title = "The Area Underneath a Normal Distribution",
       ...
       x = "") +
scale_x_continuous(breaks = c(-4, -2.58, -1.96, -1.65, -1, 0,
                              1, 1.65, 1.96, 2.58, 4),
                   labels = expression(-4*sigma, -2.58*sigma, -1.96*sigma,-1.65*sigma, 
                                       -1*sigma, mu, 1*sigma, 1.65*sigma, 1.96*sigma, 2.58*sigma, 4*sigma)) +
...
plot(x,y, main="Standard Normal Distribution", type = "l", lwd = 2,  xlab = "Z", ylab = "Probability Density", frame.plot = FALSE)
axis(1, at = -4:4)
...
If a certain man has a diastolic blood pressure of 100, we would calculate their z-score (@eq-zd) to be:
...
To find the area under the curve between two z-scores, $z_1$ and $z_2$, we have to integrate the pdf @eq-zd as following:

$$ E=P(z_1\leq Z\leq z_2)={\frac {1}{{\sqrt {2\pi }}}}\int_{z_1}^{z_2}e^{-{\frac {1}{2}}z^2}dz $$ {#eq-integral}
...
type='l', ylab="Probability Density", xlab="z",lwd=3,col="#522d80", frame.plot = F)
#abline(h=0) # adds the x-axis to the plot
polygon(region.x, region.y, density=-1, col="#F66733")

```

One solution is to use the @eq-integral with $z_1$ = 1 and $z_2$ = 2:
...
curve(dnorm(x), -4, 4, xlab = "t", ylab = "Probability Density", frame.plot = FALSE)
curve(dt(x, df = 3), col = "red", add = TRUE)
axis(1, at = -4:4)
text(0, 0.41, expression("standard normal"))
text(0, 0.34, expression(t(3)))
...
The chi-squared distribution (or $\chi^2$-distribution) with n degrees of freedom is the distribution of a sum of the squares of n independent Standard Normal random variables.

$$X^2 =\sum _{i=1}^{n}Z_{i}^{2}$$ {#eq-chi}
...
#| fig-width: 8.0
#| fig-align: "center"
#| fig-cap: chi-squared distribution with df=10, 15, and 20

curve(dchisq(x, df = 10), from = 0, to = 50, xlab = "chi-squared", ylab = "Probability Density", frame.plot = FALSE)

curve(dchisq(x, df = 15), from = 0, to = 50, col = "red", add = TRUE)
curve(dchisq(x, df = 20), from = 0, to = 50, col = "green", add = TRUE)
axis(2, at = seq(0, 01, by= 0.02))
text(10, 0.1, expression("df=10"))
text(15, 0.08, expression("df= 15"))
text(24, 0.06, expression("df=20"))
...
#| fig-width: 8.0
#| fig-align: "center"
#| fig-cap: F-distribution with different df1 and df2


curve(df(x, df1 = 2, df2 = 1), from = 0, to = 5, col= 'black', xlab = "F", ylab = "Probability Density", frame.plot = FALSE)
curve(df(x, df1 = 4, df2 = 6), from = 0, to = 5, col= 'red',
      add = TRUE)
curve(df(x, df1 = 12, df2 = 12), from = 0, to = 5, col= 'green',
      add = TRUE)

axis(2, at = seq(0, 1, by= 0.2))
text(0.5, 0.9, expression("df1=2, df2=1"))
text(1.1, 0.6, expression("df1=4, df2=6"))
text(1.3, 0.8, expression("df1=12, df2=12"))
...
(a) **Histogram**: If the data are approximately normally distributed, the shape of the histogram is similar to the normal distribution curve.

(b) **Shape statistics**: One way of measuring non-Normality **skewness** and **kurtosis** are two statistical moments for assessing Normality.

(c) **Q-Q plot**: A graphical method for comparing two probability distributions by plotting their quantiles against each other.
...
::: {.callout-note icon="false"}
## `r fa("cube", fill = "steelblue")`  **Example-Bell-shaped Empirical Distribution**
...
#| fig-width: 8.0
#| fig-align: "center"
#| fig-cap: Empirical relative frequency distribution of the birthweight.


h<-hist(data_birthweight$birthweight, breaks= 30, plot=F)
h$counts <- h$counts / sum(h$counts)
plot(h, freq=TRUE, main="Empirical distribution", xlab = "Birthweight (in Kg)", ylab="Relative Frequency", frame.plot = FALSE)
...
### Shape statistis and normality

There are two shape statistics that can indicate deviation from normality: **skewness** and **kurtosis**.

**A. Skewness**

Skewness is usually described as a measure of a dataset's symmetry -- or lack of symmetry. Skewness values that are **negative** indicate a tail to the **left** (@fig-skew2 a), **zero** value indicate a **symmetric** distribution (@fig-skew2 b), while values that are **positive** indicate a tail to the **right** (@fig-skew2 c).
...
plot(x,y1, main="Platykurtic distribution", type = "l", lty = 3, lwd = 2,  xlab = "", ylab = "Probability Density", col="grey50", xaxt='n', frame.plot = FALSE)
axis(1, at = -4:4, lab=expression(-4*sigma, -3*sigma, -2*sigma,-1*sigma, mu, 1*sigma, 2*sigma, 3*sigma, 4*sigma))
lines(x,y3,col="deeppink", lwd = 3)
...
plot(x,y1, main="Normal distribution", type = "l", lwd = 2,  xlab = "", ylab = "Probability Density", col="deeppink", xaxt='n', frame.plot = FALSE)
axis(1, at = -4:4, lab=expression(-4*sigma, -3*sigma, -2*sigma,-1*sigma, mu, 1*sigma, 2*sigma, 3*sigma, 4*sigma))


plot(x,y1, main="Leptokurtic distribution", type = "l", lty = 3, lwd = 2,  xlab = "", ylab = "Probability Density", col="grey50", xaxt='n', ylim=c(0,0.8), frame.plot = FALSE)
axis(1, at = -4:4, lab=expression(-4*sigma, -3*sigma, -2*sigma,-1*sigma, mu, 1*sigma, 2*sigma, 3*sigma, 4*sigma))
lines(x,y2,col="deeppink", lwd = 3)
...
#| fig-width: 8.0
#| fig-align: "center"
#| fig-cap: Q-Q plot of Birthweight.


qqnorm(data_birthweight$birthweight, pch = 1, frame = FALSE)
qqline(data_birthweight$birthweight, col = "steelblue", lwd = 2)

```

::: {.callout-tip icon="false"}
## `r fa("highlighter", fill = "steelblue")` Properties of an approximately bell-shaped empirical distribution
...
-   "nearly all" values (99.7%) are within -3 and +3 standard deviations of the mean (@fig-normal_auc)

-   the measure of skewness takes values close to zero (symmetric distribution) (@fig-skew2 b). Particularly, values between −1 and +1 indicate an approximately bell-shaped curve.
...
-   In Q-Q plot (@fig-qq) the points fall close to the **straight line**.
:::


 

## `r fa("compass-drafting", fill = "steelblue")` Exercises {.unnumbered}
...
Find the z -scores for $x_1$=325 and $x_2$=366.21. Interpret each z -score. What can you say about $x_1$=325 and $x_2$=366 ?

::: {.callout-note collapse="true" icon="false"}
## **Solution**

The z-score (@eq-z) for x1=325 is $z_1$=--1.5 .

The z-score (@eq-z) for x2=366 is $z_2$=--1.14 .
