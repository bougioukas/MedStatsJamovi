...
from the top menu, as shown below in @fig-diet0.

![In the Analyses Tab select Exploration and click on Descriptives.](images/diet0.png){#fig-diet0 width="75%"}

The `Descriptives` dialogue box opens. Drag the variable `WeightLoss` into the `Variables` box and split it by the `Diet` variable. Additionally, select `Variable across rows`, as shown below (@fig-diet1):

![Split the variable WeightLoss by Diet group](images/diet1.png){#fig-diet1 width="75%"}
...
![Descriptive statistics of WeightLoss by Diet group (click on image to zoom in).](images/diet2.png){#fig-diet2}
...
![Conducting ANOVA test.](images/diet4.png){#fig-diet4 width="75%"}

The `One-Way ANOVA` dialogue box opens. Drag and drop the numeric variable `WeightLoss` to Dependent Variables and the independent variable `Diet` to Grouping Variable, as shown below @fig-diet5:

![One-Way ANOVA dialogue box](images/diet5.png){#fig-diet5 width="75%"}
...
Since p = 0.583 \> 0.05, the $H_0$ of the Levene's test is not rejected and **we have to perform the Fisher's test (assume equal variances)** (@fig-diet8). (NOTE: If the $p \geq 0.05$, then the population variances of WeightLoss in all groups are assumed equal).

![Fisher's ANOVA test.](images/diet8.png){#fig-diet8 width="75%"}

If the assumption of equal variances is not satisfied (Levene's test gives p \< 0.05, reject $H_0$), the Welch's test should be used from the available Tests in Jamovi (@fig-diet8).

Next, we can inspect again the results in the group descriptives table (@fig-diet9) and pertinent plots (@fig-diet9):

![Group descriptives.](images/diet9.png){#fig-diet9 width="65%"}

![Plot of mean (95% CI) of WeightLoss by Diet.](images/diet10.png){#fig-diet10 width="55%"}

Finally, we present the results of the Fisher's ANOVA test in the table of the @fig-diet11:

![The results of the Fisher's ANOVA test.](images/diet11.png){#fig-diet11 width="55%"}

In @fig-diet11, F= 6.12 indicates the obtained F-statistic= (variation between sample means $/$ variation within the samples). Note that we compare this value to an F-distribution (F-test). The degrees of freedom in the numerator (df1) and the denominator (df2) are 3 and 56, respectively (numarator: variation between sample means; denominator: variation within the samples).
...
![Select the appropriate post-hoc test.](images/diet12.png){#fig-diet12 width="70%"}
...
![Table with the results of the post-hoc tests.](images/diet13.png){#fig-diet13 width="70%"}
...
from the top menu, as shown below in @fig-athletes0.

![In the Analyses Tab select Exploration and click on Descriptives.](images/athletes0.png){#fig-athletes0 width="75%"}

The Descriptives dialogue box opens. Drag the variable `VO2max` into the `Variables` box and split it by the `VO2max` variable. Additionally, select `Variable across rows`, as shown below (@fig-athletes1):

![Split the variable VO2max by sport group.](images/athletes1.png){#fig-athletes1 width="75%"}

We can now select the relevant descriptive statistics such as `Percantiles`, `Skewness`, `Kurtosis` and the `Shapiro-Wilk` test from the `Statistics` section (@fig-descriptives13):

![In the Statistics section select the descriptive statistics of interest.](images/descriptives13.png){width="70%"}
...
Next, in the `Plots` section we can visualize the distribution of VO2max for the three groups. Check the boxes of `Box Plot`, `Violin`, `Data` and `Mean` from `Box Plots`, as shown below (@fig-athletes3):

![In the Plot section select Box Plots.](images/athletes3.png){#fig-athletes3 width="75%"}

A graph (@fig-athletes4) is generated in the output window on our right-hand side:

![In the Plots section select Density from Histograms.](images/athletes4.png){#fig-athletes4 width="70%"}
...
We will perform Kruskal-Wallis to test the null hypothesis that the median $VO2_{max}$ is the same for all sports.
...
![Conducting Kruskal-Wallis test.](images/athletes5.png){#fig-athletes5 width="75%"}
...
![One-Way ANOVA (Non-parametric) dialogue box](images/athletes6.png){#fig-athletes6 width="75%"}
