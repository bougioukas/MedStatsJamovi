...
For example, @tbl-corn presents a typical summary table of the basic characteristics (variables) of patients entered into @farndon2013 randomized controlled trial (RCT). This study investigated the effectiveness of salicylic acid plasters compared with usual scalpel debridement for treatment of foot corns.

::: {.callout-note icon="false"}
## Example: Summary table with characteristics of patients [@farndon2013]
...
::: {.callout-note icon="false"}
## Example - Distribution of treatment center for 202 patients with foot corns [@farndon2013]

One of the categorical variables recorded in @farndon2013 study was the treatment center. Trial participants were treated at one of seven centers and the corresponding categories as displayed in @tbl-centers1. The first column shows category (treatment center) names, whilst the second shows the number of individuals in each category together with its percentage contribution to the total. @tbl-centers1 clearly shows that the majority (54.5%) of patients were treated at the "Central" treatment center.
...
::: {.callout-note icon="false"}
## Example - Distribution of treatment center by randomized group for 202 patients with foot corns [@farndon2013]
...
: Reporting numbers and percentages [examples relevant to the data provided in @farndon2013 study] {#tbl-rules1}
:::
...
If the data are binary and are coded 0 or 1, then $\bar{x}$ is the proportion of individuals with value 1, and this can also be expressed as a percentage. In @farndon2013 data, the cases in which the corn was healed are coded as '1s' and the cases in which the corn was not healed as '0s'. The corn had healed in 52 out of 189 patients (0.28 or 28%), which is equal to the "mean" of this variable `r round(mean(data_corn$complete_healed, na.rm=T), digits = 2)`.
...
4.  has a known sampling distribution (see @sec-estimation)
...
-   If the distribution is **symmetric** (mean=median=mode) (@fig-skew b), then in general the **mean** is the better summary statistic (see also @sec-distributions).

-   If the distribution is **skewed** to the left (@fig-skew a) or right (@fig-skew c) then the **median** is less influenced by the tails (see also @sec-distributions).

```{r}
#| echo: false
#| label: fig-skew
#| fig-cap: Types of distribution according to the symmetry.
#| fig-subcap: 
...
curve(dbeta(x, 2, 7), col="green", type="l",lwd = 2, xlab="x", ylab="Beta Density", frame.plot = FALSE)
segments(x0 = 0.15, x1 = 0.15, y0 = 0, y1 = 3.1, lty = 2, lwd = 2, col = "orange") 
text(0.15, 3.25, expression("mode"), font =2, cex = 1.5, col = "orange")
...
text(0.35, 1.95, expression("mean"), font=2, cex = 1.5, col = "black")
       

curve(dbeta(x, 7, 7), col="green", type="l", lwd = 2, xlab="x", ylab="Probability distribution", frame.plot = FALSE)

segments(x0 = 0.45, x1 = 0.45, y0 = 0, y1 = 2.9, lty = 2, lwd = 2, col = "orange") 
text(0.4, 2.85, expression("mode"), font =2, cex = 1.5, col = "orange")

segments(x0 = 0.5, x1 = 0.5, y0 = 0, y1 = 3.0, lty = 2, lwd = 2, col = "black") 
text(0.5, 3.0, expression("mean"), font = 2, cex = 1.5, col = "black")

segments(x0 = 0.55, x1 = 0.55, y0 = 0, y1 = 2.8, lty = 2, lwd = 2, col = "blue") 
text(0.63, 2.9, expression("median"), font=2, cex = 1.5, col = "blue")
       


curve(dbeta(x, 7, 2), col="green", type="l", lwd = 2, xlab="x", ylab="Beta Density", frame.plot = FALSE)
...
text(0.73, 2.7, expression("median"), font=2, cex = 1.5, col = "blue")

segments(x0 = 0.85, x1 = 0.85, y0 = 0, y1 = 3.1, lty = 2, lwd = 2, col = "orange") 
text(0.88, 3.1, expression("mode"), font =2, cex = 1.5, col = "orange")
...
**B. Median (Q1, Q3)** for those with **skewed** (or asymetrical) distribution.  
...
: Recommendations for reporting summary statistics [examples relevant to the data provided in @farndon2013 study] {#tbl-rules2}
:::
...
Choice of number of bins has an inverse relationship with the bin width @fig-histo2. The smaller the number of bins, the larger bin width there will be to cover the whole range of data. 
...
#| warning: false
#| label: fig-histo2
#| fig-cap: Histogram of age for different number of bins (a) bins = 50, (b) bins = 15, and (c) bin width = 8.
#| fig-width: 8.5
#| fig-height: 4.5
...
p1 <- data_corn |>
  ggplot(aes(x = age)) +
  geom_histogram(bins=50, fill="steelblue4", color="#8fb4d9", alpha=0.6) +
  ggtitle("Number of bins = 50") +
  labs(y="Frequency") +
  theme_minimal() +
  theme(
...
p2 <- data_corn |>
  ggplot(aes(x = age)) +
  geom_histogram(bins=15, fill="steelblue4", color="#8fb4d9", alpha=0.6) +
  ggtitle("Number of bins = 15") +
  labs(y="Frequency") +
  theme_minimal() +
  theme(
      plot.title = element_text(size=14),
...
p3 <- data_corn |>
  ggplot(aes(x = age)) +
  geom_histogram(bins=8, fill="steelblue4", color="#8fb4d9", alpha=0.6) +
  ggtitle("Number of bins = 8") +
  labs(y="Frequency") +
  theme_minimal() +
  theme(
      plot.title = element_text(size=14),
...
#| warning: false
#| label: fig-histo3
#| fig-cap: Histogram of age of participants  by treatment group in @farndon2013 study.
#| fig-width: 7.5
#| fig-height: 5.0
...
#| warning: false
#| label: fig-raincloud
#| fig-cap: A raincloud plot of age of the participants by treatment group in @farndon2013 study.
#| fig-width: 6.5
#| fig-height: 4.0
...
## Describing and displaying associations between two continuous variables

It is convenient to use a single number to summarize a steadily increasing or decreasing association between two variables. A statistic that involves two continuous variables is the **correlation coefficient**. This idea originally proposed by Francis Galton but formally published in 1895 by Karl Pearson, one of the founders of modern statistics. The concept of correlation implies that two variables covary. That is, **a change in variable X is associated with a change in variable Y**.

::: {.callout-tip icon="false"}
## Correlation coefficient

Correlation coefficient is a **descriptive measure** of the degree of association between two continuous variables.
:::

We present two of the most important correlations coefficients in @sec-correlation.

Visual inspection of a plot of coordinate points for the X and Y variables is necessary. Such a plot is called a **scatter plot** and is illustrated in @fig-scatterplot.

```{r}
#| echo: false
#| warning: false
#| label: fig-scatterplot
#| fig-cap: Association between (a) age and baseline pain, and (b) age and baseline pain taking into acound the gender of the participants in @farndon2013 study.
#| fig-width: 8.5
#| fig-height: 4.0
...
Scatter plots are **two dimensional** graphs, showing the association between two continuous variables such as age and baseline pain (@fig-scatterplot a). By coloring the data by a third categorical variable (here is gender), scatter plots can provide additional dimensions (@fig-scatterplot b).
:::
